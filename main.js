...
function displayExp(details) {
  ...
  for (i = 0; i < details.length; i++) {
    ...
    <div class="expValue" id="${details[i].id}">
      ...
      <div id="edite_delete">
        <p>
          <button id="${details[i].id}" onclick="editExpDetails(${details[i].id})"> <img src="image/edit.jpeg" width="15" alt=""  /></button> 
          <button id="${details[i].id}" onclick="delExpenseDetails(${details[i].id})"><img src="image/trash.jpeg" width="15" alt="" /></button>
        </p>
      </div>
    </div>
  `;
